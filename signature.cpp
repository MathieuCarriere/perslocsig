...
int main(int argc, char** argv) {
	...
	if (argc <= 1){ cout << "Inputs: <PD name> <SC/AGD> (<dimension>)" << endl; return 0; }
	...
	if ((*p)==*("g"))
		if ((*f)==*("e")){
			if (strcmp(signature,"SC")==0)
	    			sprintf(sign,"signature_ecc_points_%s", (char*) id.c_str());
			if (strcmp(signature,"AGD")==0)
				sprintf(sign,"topoAGD_ecc_points_%s", (char*) id.c_str());
		}
	if ((*p)==*("l")){
		if ((*f)==*("g")){
			if (strcmp(signature,"SC")==0)
			...
			if (strcmp(signature,"AGD")==0)
				sprintf(sign,"topoAGD_geo_points_%s", (char*) id.c_str());
		}
        	if ((*f)==*("h")){
			if (strcmp(signature,"SC")==0)
	    			sprintf(sign,"signature_hks_points_%s", (char*) id.c_str());
			if (strcmp(signature,"AGD")==0)
				sprintf(sign,"topoAGD_hks_points_%s", (char*) id.c_str());
		}
	}
	...
	if (strcmp(signature,"AGD")==0){
        	int dim = 1; SIGN.write((char*) &dim, 4); PD.clear(); PD.seekg(0);}
		...
		if (k%1000 == 0)
			...
			if (strcmp(signature,"SC")==0){
				...
				cm.push_back(log(1+v[0])); int n = cm.size();
			...
			if (strcmp(signature,"AGD")==0){
				...
				cm.push_back(log(1+v[0])); double somme = cm[0];
				SIGN.write((char*) &somme, 8);
